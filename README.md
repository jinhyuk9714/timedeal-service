...
원본 README와 `docs/perf/` 문서에 남아 있던 대표 결과입니다.

| 시나리오 | 결과 |
| --- | --- |
| 기본 READ | 약 40 RPS, 에러율 0%, p95 < 50ms |
| 타임딜 스파이크 - 재고 부족 | 약 165 RPS, p95 약 950ms, 데이터 정합성 유지 |
| 타임딜 스파이크 - 재고 충분 | 약 160 RPS, 주문 6,505건 201 성공, 에러율 0% |
| Soak 혼합 부하 | 10 VU 10분, 에러율 0%, p95 약 118ms |
| 캐시 도입 | p95 38.8ms -> 11.6ms, 약 70% 개선 |
| 락 전략 비교 | 비관적 락 0% 실패, 낙관적 락 23% 실패, Redis 분산 락 42.61% 실패 |
| 상품 분산 주문 | 약 247 RPS, p95 약 863ms, 주문 6,685건 201 성공 |
